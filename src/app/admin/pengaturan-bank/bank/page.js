...
export default function BankManagementPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Kita pastikan state banks benar-benar baru
      if (data) {
        setBanks([...data]); 
      }
    } catch (error) {
  ...
  // 3. Efek saat halaman dibuka
  useEffect(() => {
    loadData();
    
    // Trik tambahan: Refresh data setiap kali user kembali ke tab ini
        ...
        <div className="p-4 relative">
          <div className="flex justify-end mb-4">
            <button 
              ...
              className="bg-[#198754] hover:bg-green-700 text-white px-3 py-1.5 rounded text-sm flex items-center gap-1 transition-all"
            >
            ...
            <table className="w-full border-collapse border border-gray-200">
              <thead>
                <tr className="bg-white text-gray-700 text-[13px] font-bold">
                  ...
                  <th className="border p-2 text-left font-bold text-gray-400 font-light italic">Status</th>
                  <th className="border p-2 text-center font-bold">Gambar</th>
              ...
              <tbody className="text-[13px]">
                {loading ? (
                  <tr><td colSpan="8" className="p-10 text-center italic text-gray-400">Memuat data terbaru...</td></tr>
                ) : banks.length > 0 ? (
                  banks.map((bank, index) => (
                    <tr key={bank.id} className="hover:bg-gray-50 transition-colors border-b">
                      ...
                      <td className="border p-3 text-center">
                        {bank.img ? (
                           <img 
                             key={`${bank.id}-${bank.img}`}
                             src={`${bank.img}${bank.img.includes('?') ? '&' : '?'}t=${Date.now()}`} 
                             alt={bank.nama} 
                      ...
                      <td className="border p-3 text-center">
                         <div className={`w-3 h-3 mx-auto rounded-full ${bank.register ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      </td>
                      <td className="border p-3 text-center">
                         <div className={`w-3 h-3 mx-auto rounded-full ${bank.deposit ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      </td>
                      <td className="border p-3 text-center">
                        <button 
                  ...
                  <tr>
                    <td colSpan="8" className="p-10 text-center text-gray-400 italic font-light">
